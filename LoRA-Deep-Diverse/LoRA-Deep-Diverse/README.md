...
Identical hyperparameters are used in both training runs to ensure a fair comparison.
...
For each training variant, multiple checkpoints are evaluated.
The checkpoint with the highest Pass@1 score is selected as the best-performing model.
...
Conclusion
The experimental results demonstrate that the diverse_instruction model achieves superior generalization performance under identical training conditions. This outcome highlights the importance of dataset diversity in LoRA-based fine-tuning for code generation tasks.
